...
# Assign unique node IDs based on unique XY combinations
nodes <- nodes %>%
  mutate(xy = paste(X, Y)) %>%
  mutate(nodeID = group_indices(., factor(xy, levels = unique(xy)))) %>%
  select(-xy)

# Create from–to columns for the edges
source_nodes <- nodes %>% 
...
# Map the graph
ggplot() +
  geom_sf(data = edges, color = "gray", size = 0.5) +
  geom_sf(data = nodes, color = "blue", size = 2) +
...
graph <- graph %>%
  ...
  mutate(betweenness = centrality_betweenness(weights = edge_length)) %>%
  activate(edges) %>%
  mutate(betweenness = centrality_edge_betweenness(weights = edge_length))
...
betweeness_centrality_graph <- ggplot() +
  ...
  geom_sf(data = graph %>%
            ...
            as_tibble() %>%
            st_as_sf(), 
          aes(color = betweenness, size = betweenness)) +
  scale_colour_viridis_c(option = 'inferno') +
  scale_size_continuous(range = c(0, 4)) +
  labs(title = "Betweenness Centrality of Water Systems in NC") +
  theme_minimal()
...
degree_centrality_graph <- ggplot() +
  ...
  scale_color_viridis_c(option = "inferno", name = "Degree") +
  scale_size_continuous(range = c(0, 4), name = "Degree") +
...
# Define locations
chapel_hill_pt <- st_sfc(st_point(c(-79.05, 35.91)), crs = 4326)
cary_pt <- st_sfc(st_point(c(-78.78, 35.78)), crs = 4326)
raleigh_pt <- st_sfc(st_point(c(-78.64, 35.78)), crs = 4326)
...
chapel_hill_node <- nodes %>% 
  slice(st_nearest_feature(chapel_hill_pt, nodes)) %>% 
  pull(nodeID)

cary_node <- nodes %>% 
  slice(st_nearest_feature(cary_pt, nodes)) %>% 
  pull(nodeID)

raleigh_node <- nodes %>% 
  slice(st_nearest_feature(raleigh_pt, nodes)) %>% 
  pull(nodeID)
...
shortest_cary_distance <- path_cary_graph %>%
  activate(edges) %>%
  as_tibble() %>%
  summarize(total_distance_m = sum(edge_length))
...
shortest_raleigh_distance <- path_raleigh_graph %>%
  activate(edges) %>%
  as_tibble() %>%
  summarize(total_distance_m = sum(edge_length))

print(paste("Shortest distance from Raleigh to OWASA", round(shortest_raleigh_distance, 2), "meters away."))

print(paste("Shortest distance from Cary to OWASA", round(shortest_cary_distance, 2), "meters away."))
